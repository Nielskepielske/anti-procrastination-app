...
private fun getLinuxActiveApp(): WindowInfo? {
    val sessionType = System.getenv("XDG_SESSION_TYPE")?.lowercase() ?: ""
    val desktop = System.getenv("XDG_CURRENT_DESKTOP")?.lowercase() ?: ""
...
// --- HYPRLAND ---
private fun getHyprlandActiveApp(): WindowInfo? {
    return try {
        ...
        while (reader.readLine().also { line = it } != null) {
            ...
            // Extract the Window Address from the first line (e.g. "Window 55fb4e654e70 -> ...")
            if (trimmed.startsWith("Window ") && trimmed.contains("->")) {
                val addressPart = trimmed.substringAfter("Window ").substringBefore(" ->").trim()
                if (addressPart.isNotEmpty()) {
                    activeAddress = "0x$addressPart"
                }
            }
            ...
            if (trimmed.startsWith("class:")) {
                ...
                activeTitle = trimmed.removePrefix("title:").trim()
            } else if (trimmed.startsWith("initialTitle:")) {
                activeInitialTitle = trimmed.removePrefix("initialTitle:").trim()
            }
        }
        process.waitFor()
        
        if (activeClass.isNotEmpty()) {
            val bestTitle = if (activeInitialTitle.length > activeTitle.length && !activeInitialTitle.matches(Regex("win\\d+"))) {
                activeInitialTitle
            } else if (!activeTitle.matches(Regex("win\\d+"))) {
                activeTitle
            } else {
                activeInitialTitle.ifEmpty { activeTitle.ifEmpty { activeClass } }
            }
            WindowInfo(activeClass, bestTitle, activeAddress)
        } else {
            null
        }
...
// --- KDE WAYLAND ---
private fun getKdeActiveApp(): WindowInfo? {
    return try {
        ...
        process.waitFor()
        activeWindow?.takeIf { it.isNotEmpty() && it != "Desktop" }?.let { WindowInfo(it, it, "") }
    } catch (e: Exception) { null }
}

// --- X11 (Standard Linux Fallback) ---
private fun getX11ActiveApp(): WindowInfo? {
    return try {
...
private fun getMacActiveApp(): WindowInfo? {
    return try {
        ...
        process.waitFor()
        activeApp?.takeIf { it.isNotEmpty() }?.let { WindowInfo(it, it, "") }
    } catch (e: Exception) { null }
}

private fun getWindowsActiveApp(): WindowInfo? {
    return try {
        val script = """
            Add-Type @"
              using System;
